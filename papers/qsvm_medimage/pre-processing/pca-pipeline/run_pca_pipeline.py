...
def main():
    ...
    for file_num in available_files:
        ...
        if args.verify_only:
            if not output_exists:
                row["status"] = "skipped"
                row["verified"] = False
                print("  Output not found, skipping verification")
                summary_data.append(row)
                continue
        elif output_exists and not args.force:
            print("  Output already exists, skipping (use --force to reprocess)")
    ...
    verified = sum(1 for r in summary_data if r.get("verified"))
